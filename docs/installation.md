# NGI-CellRaiser Installation

To start using the NGI-CellRaiser pipeline, there are three steps described below:
...
This pipeline itself needs no installation - NextFlow will automatically fetch it from GitHub if `SciLifeLab/NGI-CellRaiser` is specified as the pipeline name.
...
```bash
wget https://github.com/SciLifeLab/NGI-CellRaiser/archive/master.zip
unzip master.zip -d /my-pipelines/
cd /my_data/
nextflow run /my-pipelines/ngi-cellraiser-master
```
